...
public class DrawConceptCombo extends JPanel
{
	...
	// Ajout pour choisir la méthode de classification non supervisée
	private String[] methodesNonSupervisees = { "Sélectionnez une méthode", "K-Means"};
		public JComboBox jcb_lesMethodesNonSupervise = new JComboBox(methodesNonSupervisees);
		
		public JLabel lbl_nbClusters = new JLabel("Clusters (C) : ");
		public JTextField nbClusters = new JTextField("c", 5);
		
		public JLabel lbl_nbIteration = new JLabel("Itérations (N) : ");
	 ...
	 */
	public DrawConceptCombo(Image inputImage, String title) {
	 ...
	 */
	public DrawConceptCombo(Image inputImage, String title, Image displayImage) {
...
//		jPanel3.add(createArff);
		...
		jPanel5.add(jcb_lesMethodes);
			...
			jcb_lesMethodesNonSupervise.setSelectedIndex(0);
		jPanel5.add(jcb_lesMethodesNonSupervise);
		
		jPanel5.add(lbl_nbClusters);
		jPanel5.add(nbClusters);
		
		jPanel5.add(lbl_nbIteration);
		jPanel5.add(nbIteration);
...
// Ajout d'une gomme :
	...
	public void okButton_actionPerformed(ActionEvent e) 
	{
		...
		else 
		{
			...
			// Création du fichiers arff :
				...
				boolean bonneValeurN = false;
				
				int nbN;
				nbN = 1;
				
				boolean bonneValeurC = false;
				
				int nbC;
				nbC = nomConcept.size() - 1;
				
				if(this.nbIteration.getText() != "n")
				{
					try
					{
						nbN = Integer.parseInt(this.nbIteration.getText());
					...
					{
						JOptionPane.showMessageDialog(this,
				   			    "Vous devez entrer un entier pour N",
				   			    "Information",
				   			    JOptionPane.INFORMATION_MESSAGE);
					}
				}
				...
				{
					JOptionPane.showMessageDialog(this,
			   			    "Veuillez entrer la valeur de N",
			   			    "Information",
			   			    JOptionPane.INFORMATION_MESSAGE);
				}
				if(this.nbClusters.getText() != "c")
				{
					try
					{
						nbC = Integer.parseInt(this.nbClusters.getText());
					...
					{
						JOptionPane.showMessageDialog(this,
				   			    "Vous devez entrer un entier pour C",
				   			    "Information",
				   			    JOptionPane.INFORMATION_MESSAGE);
					}
				}
				...
				if((bonneValeurN == true) && (bonneValeurC == true))
				{
					Instances dataSet_sansClasse = CreerArffSansClasseTexte();
					dataSet_sansClasse.setClassIndex(dataSet_sansClasse.numAttributes()-1);
					
					Instances dataSet_avecClasse = CreerArffAvecClasseTexte();
					dataSet_avecClasse.setClassIndex(dataSet_avecClasse.numAttributes()-1);
					
					int[] resultat;
					resultat = new int[dataSet_sansClasse.numInstances() + 1 ];
					...
					if(jcb_lesMethodes.getSelectedItem() == "Naive Bayes")
					{
						...
						this.CreerArffAvecTableauClass(resultat);
					}
					else if(jcb_lesMethodes.getSelectedItem() == "J48")
					{
						J48 tree = new J48();
						try
						{
					...
					else if(jcb_lesMethodes.getSelectedItem() == "IBk")
					{
			...
			// On créé un dataSet sans classe pour KMeans
		...
		/** On effectue les N itérations de Kmeans */
					int[][] KMeansResultat;
			    	KMeansResultat = new int[nbN][dataSet.numInstances()];
				...
				for(int n = 0; n < nbN; n++)
				{
					...
					try
					{
						// On enlève la classe "gomme" qui joue un rôle particulier
						kmeans.setNumClusters(nbC);
					}
					catch (Exception ex)
					{
					...
					try
					{
						kmeans.setSeed(nb.nextInt(2000));
					}
					catch (Exception ex)
					{
						System.out.println(ex.toString());
					...
					this.CreerArffAvecTableauClassEtCluster(resultat, KMeansResultat, nbN, nbC);
					System.out.println("Réussite");
				// Fin if bonne valeur de k
			}
	...
	public class Draw2D_brushSpinner_actionAdapter implements javax.swing.event.ChangeListener
	{
		...
		Draw2D_brushSpinner_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_undoButton_actionAdapter implements	java.awt.event.ActionListener
	{
		...
		Draw2D_undoButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_labelComboBox_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_labelComboBox_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_okButton_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_okButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_resetButton_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_resetButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_plusButton_actionAdapter implements	java.awt.event.ActionListener
	{
		...
		Draw2D_plusButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_modifyLabel_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_modifyLabel_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_createArffButton_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_createArffButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_gommeButton_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_gommeButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_drawAnAreaButton_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_drawAnAreaButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_saveButton_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_saveButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_openButton_actionAdapter implements java.awt.event.ActionListener
	{
		...
		Draw2D_openButton_actionAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Draw2D_transparencySlider_changeAdapter implements javax.swing.event.ChangeListener
	{
		...
		Draw2D_transparencySlider_changeAdapter(DrawConceptCombo d2d)
		{
			this.d2d = d2d;
		}
	...
	public class Selection_changeAdapter implements ActionListener 
	{
	    ...
	    public Selection_changeAdapter(DrawConceptCombo ihm)
	    {
	    	this.ihm = ihm;
	    }
		 ...
		 */
		class MarkerDisplayJAII extends DisplayJAI {
	...
	// ------------------	
			...
			MarkerDisplayJAII(DrawConceptCombo monIHM) {
				super();
				setCursor(Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR));
		...
		// Le polygone est terminé :
			...
			void colorier(int x, int y)
			{
				...
				if(this.IHM.output.getPixelXYBByte(x, y-1, 0) != IHM.labelComboBox.getSelectedIndex()) 
				{
				    colorier(x, y-1);
				}
				if(this.IHM.output.getPixelXYBByte(x-1, y, 0) != IHM.labelComboBox.getSelectedIndex()) 
				{
				    colorier(x-1, y);
				}
				if(this.IHM.output.getPixelXYBByte(x, y+1, 0) != IHM.labelComboBox.getSelectedIndex()) 
...
*/
	...
	public void CreerArffAvecTableauClass(int[] resultat)
	{
	    Instances dataSet;
	    // Vecteur représentant une ligne = un pixel avec ses valeurs en RGB donnés par des vecteurs de niveau inférieur.
	    FastVector wekaAttributes = new FastVector();;
		...
		for(int y = 0; y < ydim; y++)
		{
			for(int x = 0; x < xdim; x++)
			{
					...
					// On a ajouté les valeurs pour chaque bande, on peut maintenant ajouter la valeur de la classe :
					instEx.setValue((Attribute)wekaAttributes.elementAt(bdim), (String) nomConcept.get(resultat[x + y * xdim]));
		...
		// Enregistrement des données :
		try {
			saveData(dataSet, "./newModule.arff");
 		}
		catch (Exception ex) {
			System.out.println(ex.toString());
		}
	...
	public void CreerArffAvecTableauClassEtCluster(int[] resultat, int[][] KMeansResultat, int nbN, int nbC)
	{
	    Instances dataSet;
	    // Vecteur représentant une ligne = un pixel avec ses valeurs en RGB donnés par des vecteurs de niveau inférieur.
	    FastVector wekaAttributes = new FastVector();
	...
	/** On ajoute autant d'Attribut 2 qu'on veut faire d'itérations de Simple KMeans */
		FastVector clusterVector;
		for(int n = 0; n < nbN; n++)
		{
			clusterVector = new FastVector();
			for (int c = 0; c < nbC; c++)
			{
				clusterVector.addElement(String.valueOf(c));
			}
			// Pour chaque itération de Kmeans on créé un argument de type numérique qui contiendra l'intensité
			wekaAttributes.addElement(new Attribute("n" + (n+1), clusterVector));
		}   
		...
		for(int y = 0; y < ydim; y++)
		{
			for(int x = 0; x < xdim; x++)
			{
					...
					for(int n = 0; n < nbN; n++)
					{
/** Ici on ajoute les N itérations de Kmeans */
						
						instEx.setValue((Attribute)wekaAttributes.elementAt(n + bdim), String.valueOf(KMeansResultat[n][x + y * xdim]));
					}
					// On a ajouté les valeurs pour chaque bande, on peut maintenant ajouter la valeur de la classe :
					instEx.setValue((Attribute)wekaAttributes.elementAt(bdim + nbN), (String) nomConcept.get(resultat[x + y * xdim]));
					// Quand on arrive là on a enregistré les valeurs du pixel dans chaque bande, on ajoute notre instance à l'ensemble
					dataSet.add(instEx);
			}
		}
		...
		// Enregistrement des données :
		try {
			saveData(dataSet, "./newModule2.arff");
 		}
		catch (Exception ex) {
			System.out.println(ex.toString());
		}
